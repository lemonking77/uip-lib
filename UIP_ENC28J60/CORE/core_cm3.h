 ...
 * Note: The priority cannot be set for every core interrupt.
 */
static __INLINE uint32_t NVIC_GetPriority(IRQn_Type IRQn)//读取中断的优先级分组
{
 ...
 */
static __INLINE uint32_t SysTick_Config(uint32_t ticks)//SysTick时钟配置
{ 
  ...
  SysTick->CTRL  = SysTick_CTRL_CLKSOURCE_Msk | 							 /* */
                   SysTick_CTRL_TICKINT_Msk   | 							 /* 使能异常中断 */
